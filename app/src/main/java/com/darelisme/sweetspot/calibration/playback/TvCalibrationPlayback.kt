...
internal class TvCalibrationPlayback(
    ...
    private fun startInternal(
        ...
        val savedState = when (override) {
            ...
            is MeasurementAudioOverrideResult.Failed -> {
                audioOperationGate.releaseTransient()
                return CalibrationAudioResult.Failure(override.error)
            }
        }
        val track = try { createTrack() } catch (error: Throwable) {
            restore(savedState, candidateId)
            audioOperationGate.releaseTransient()
            return CalibrationAudioResult.Failure(error.message ?: "Calibration playback could not start")
        }
        val next = Active(request, candidateId, track, savedState, onFinished)
        synchronized(lock) {
            if (closed || active != null) {
                try { track.release() } catch (_: Throwable) {}
                restore(savedState, candidateId)
                audioOperationGate.releaseTransient()
                return CalibrationAudioResult.Failure("Calibration playback is unavailable")
            }
            active = next
        }
        try {
            ...
            synchronized(lock) { next.stopped = true }
            finish(next)
            return CalibrationAudioResult.Failure(error.message ?: "Calibration playback could not start")
        }
    }
    ...
    private fun play(current: Active) {
        try {
            ...
            Log.w(TAG, "TV calibration sweep failed: ${error.message}")
        } finally {
            finish(current)
        }
    }
    ...
    private fun finish(current: Active) {
        synchronized(lock) {
        ...
        if (current.notifyFinished) current.finished()
    }
